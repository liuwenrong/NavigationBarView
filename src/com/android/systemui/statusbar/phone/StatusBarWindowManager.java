...
public class StatusBarWindowManager {
	...
	public static final String TAG = "xx-lwr--StatusBarWindowManager";
	Display mDisplay;
    ...
    public StatusBarWindowManager(Context context) {
        ...
        //coolpad added, disable keyguard screen rotation as required.
     ...
     */
    public void add(View statusBarView, int barHeight, WindowManagerImpl wmi) {
...
//    	   mWindowManager.
		...
		LogHelper.logE(TAG, "添加PhoneStatusBar");
//        mWindowManager.addView(mStatusBarView, mLp); // 干掉系统SystemUI的时候可以用,否则报错 another window of this type already exists.

//        WindowManagerImpl wmi = new WindowManagerImpl(mDisplay);
		wmi.addView(mStatusBarView, mLp);
...
//		vri.setView(mStatusBarView, mLp, null);
    ...
    private void applyHeight(State state) {
        ...
        if (expanded) {
			...
			if (state.coverModeShowing) {
				mLp.height = mBarHeight + mCoverTopDistance;
			} else {
    ...
    private static class State {
        ...
        boolean qsExpanded;
        boolean needsWallpaper;

        /**
         * The {@link BaseStatusBar} state from the status bar.
         */
        int statusBarState;
		public boolean coverModeShowing;
	...
	public void setCoverModeShowing(boolean showing, int top_padding) {
		mCurrentState.coverModeShowing = showing;
		mCoverTopDistance = top_padding;
		apply(mCurrentState);
	}

    public void setNeedsWallpaper(boolean needsWallpaper) {
        mCurrentState.needsWallpaper = needsWallpaper;
        apply(mCurrentState);
    }
}
